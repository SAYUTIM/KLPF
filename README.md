...
- `attendanceRate.js`
  Ku-portの出席率と最終カードタッチ日を取得・保存し、Ku-LMSホームの授業カードへ表示。キャッシュ・更新中・最新の表示状態も管理します。
- `lmsInlineSettings.js`
  Ku-LMS内のKLPF設定、テーマカラー変更、出席状況の手動更新メニューを管理。
- `customImageTheme.js`
...
Ku-portから取得したHTMLをDOMとして解析するためのOffscreen Documentです。出席フォーム、JSFの遷移情報、出席率および最終カードタッチ日をバックグラウンド処理から安全に解析します。
